...
public class ParserSerializer {
  ...
  private static final Map<String, TokenType> stringToTokenType =
      new HashMap<>() {
        {
  ...
  public static List<Token> deserializeTokens(File file) {
    ...
    while (scanner.hasNext()) {
      ...
      result.add(
          new ConcreteToken(stringToTokenType.get(values[0]), values[1], new Position(0, 0)));
    }
    return result;
  }
  ...
  public static String serializeAst(AST ast) {
    ...
    if (!ast.getLeftIf().isEmpty() || !ast.getRightIf().isEmpty()) {
      return "{"
          + ast.getLeftIf().stream()
              .map(ParserSerializer::serializeAst)
              .collect(Collectors.joining(","))
          + "}"
          + ast.getToken().getValue()
          + "{"
          + ast.getRightIf().stream()
              .map(ParserSerializer::serializeAst)
              .collect(Collectors.joining(","))
          + "}";
    }
    return "("
        + serializeAst(ast.getLeftChild())
        + ", "
        + ast.getToken().getValue()
        + ", "
