...
public class ParserSerializer {
    ...
    public static String serializeAst(AST ast){
        ...
        if(!ast.getLeftIf().isEmpty() || !ast.getRightIf().isEmpty()){
            return "{"+ast.getLeftIf().stream().map(ParserSerializer::serializeAst).collect(Collectors.joining(","))+"}"
                    + ast.getToken().getValue() +
                    "{"+ast.getRightIf().stream().map(ParserSerializer::serializeAst).collect(Collectors.joining(","))+"}";
        }
