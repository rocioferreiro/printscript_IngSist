...
//    if (tokens.stream()
//        .anyMatch(t -> t.getType().getCategory().equals(Operator.PLUS.getCategory()))) { // OPERATOR
//      int index =
//          IntStream.range(0, tokens.size())
//              .filter(
//                  t -> tokens.get(t).getType().getCategory().equals(Operator.PLUS.getCategory()))
//              .toArray()[0];
...
//      } else {
//        Variable aux =
//            new VariableBuilder().setName(tokens.get(assignationVariableIndex).getValue()).build();
