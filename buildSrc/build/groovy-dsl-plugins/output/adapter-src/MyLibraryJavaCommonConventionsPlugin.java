...
public class MyLibraryJavaCommonConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    try {
      Class<? extends BasicScript> precompiledScriptClass =
          Class.forName("precompiled_MyLibraryJavaCommonConventions").asSubclass(BasicScript.class);
  ...
  private static void assertSupportedByCurrentGradleVersion() {
    if (GradleVersion.current()
            .getBaseVersion()
            .compareTo(GradleVersion.version(MIN_SUPPORTED_GRADLE_VERSION))
        < 0) {
      throw new RuntimeException(
          "Precompiled Groovy script plugins require Gradle "
              + MIN_SUPPORTED_GRADLE_VERSION
              + " or higher");
    }
  }
}
